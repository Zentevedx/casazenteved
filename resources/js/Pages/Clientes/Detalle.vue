...
import { ArrowLeftIcon, DocumentTextIcon, PrinterIcon, PencilSquareIcon, TrashIcon, ClockIcon, CurrencyDollarIcon, BanknotesIcon, ExclamationTriangleIcon, PhotoIcon, StarIcon } from '@heroicons/vue/24/outline';
...
const props = defineProps({
    cliente: Object,
    prestamos: Array,
    desempeno: Object, // Traído del controlador
});
...
const bgMap = {
    ...
    'yellow': 'bg-amber-100 dark:bg-amber-500/10 border-amber-200 dark:border-amber-500/20',
    'orange': 'bg-orange-100 dark:bg-orange-500/10 border-orange-200 dark:border-orange-500/20',
    'red': 'bg-red-100 dark:bg-red-500/10 border-red-200 dark:border-red-500/20',
...
<template>
    ...
    <AuthenticatedLayout>
        <div class="flex h-[calc(100vh-65px)] bg-gray-50 dark:bg-[#0f0f0f] overflow-hidden font-sans selection:bg-indigo-500 selection:text-white">
            ...
            <div class="w-full md:w-80 xl:w-[400px] bg-white dark:bg-[#0f0f0f] border-r border-gray-200 dark:border-gray-800 flex flex-col z-20">
                ...
                <!-- Client Info Card -->
                <div class="p-6 border-b border-gray-200 dark:border-gray-800 flex flex-col items-center text-center bg-white dark:bg-[#141414] shadow-sm relative shrink-0">
                    ...
                    <div 
                        class="w-24 h-24 rounded-3xl bg-gradient-to-br flex items-center justify-center text-3xl font-bold text-white shadow-xl mb-4 ring-4 ring-white dark:ring-[#0f0f0f]"
                        :class="getAvatarGradient(cliente.id)"
                    >
                        {{ getInitials(cliente.nombre) }}
                    </div>
                    <h2 class="text-xl font-black text-gray-900 dark:text-white leading-tight px-4">{{ cliente.nombre }}</h2>
                    ...
                    <!-- Client Performance Score Card (NEW) -->
                    <div v-if="desempeno" :class="['mt-5 w-full rounded-2xl border p-4 text-left shadow-sm', bgMap[desempeno.color]]">
                        <div class="flex items-center justify-between mb-2">
                            <p class="text-xs font-bold uppercase tracking-widest text-gray-500 dark:text-gray-400">Desempeño</p>
                            <p :class="['text-xs font-black px-2 py-0.5 rounded-md bg-white/50 dark:bg-black/20', colorMap[desempeno.color]]">
                                {{ desempeno.score }}/100
                            </p>
                        </div>
                        <div class="flex items-center gap-1 mb-1">
                            <template v-for="i in 5">
                                <StarIconSolid v-if="i <= desempeno.estrellas" class="w-5 h-5" :class="colorMap[desempeno.color]" />
                                <StarIcon v-else class="w-5 h-5 text-gray-300 dark:text-gray-700" />
                            </template>
                        </div>
                        <p :class="['text-base font-black', colorMap[desempeno.color]]">{{ desempeno.etiqueta }}</p>
                        
                        <div class="mt-3 grid grid-cols-2 gap-2 text-xs">
                            <div class="bg-white/60 dark:bg-black/20 rounded p-1.5 text-center">
                                <p class="text-gray-500 dark:text-gray-400 font-medium">Pagados</p>
                                <p class="font-black text-gray-900 dark:text-white">{{ desempeno.metricas?.pagados ?? 0 }}</p>
                            </div>
                            <div class="bg-white/60 dark:bg-black/20 rounded p-1.5 text-center">
                                <p class="text-gray-500 dark:text-gray-400 font-medium">C/ Multa</p>
                                <p class="font-black text-gray-900 dark:text-white">{{ desempeno.metricas?.con_multa ?? 0 }}</p>
                            </div>
                        </div>
                    </div>
                ...
                <!-- Loan List -->
                <div class="flex-1 overflow-y-auto p-4 space-y-6 custom-scrollbar bg-gray-50/50 dark:bg-[#0f0f0f]">
            ...
            <div class="flex-1 bg-gray-50 dark:bg-[#0f0f0f] flex flex-col h-full overflow-hidden relative">
                ...
                <div v-if="selectedLoan" class="flex-1 overflow-y-auto p-4 md:p-8 lg:p-10 z-10 custom-scrollbar">
                    ...
                    <!-- Header Actions -->
                    <div class="flex flex-col xl:flex-row justify-between items-start xl:items-center gap-6 mb-8 pb-8 border-b border-gray-200 dark:border-gray-800">
                        <div>
                        ...
                        <div class="flex flex-wrap gap-3 w-full xl:w-auto">
                            <button @click="abrirComprobantePrestamo"
                                class="px-5 py-3 bg-white dark:bg-indigo-600/10 hover:bg-gray-50 dark:hover:bg-indigo-600/20 text-indigo-600 dark:text-indigo-400 border border-indigo-200 dark:border-indigo-600/30 rounded-xl font-bold transition shadow-sm dark:shadow-lg flex items-center justify-center gap-2 flex-1 xl:flex-none">
                                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-4l-4 4m0 0l-4-4m4 4V4"></path></svg> 
                                Imagen Recibo
                            </button>
                            <button @click="imprimirContrato"
                                class="px-5 py-3 bg-white dark:bg-[#1a1a1a] hover:bg-gray-50 dark:hover:bg-[#252525] text-gray-700 dark:text-gray-300 hover:text-gray-900 dark:hover:text-white border border-gray-200 dark:border-gray-700 rounded-xl font-bold transition shadow-sm dark:shadow-lg flex items-center justify-center gap-2 flex-1 xl:flex-none">
                            ...
                            <template v-if="['Activo', 'Vencido'].includes(selectedLoan.estado)">
                                <button @click="abrirModalPago('Interes')" 
                                    class="px-6 py-3 bg-amber-500 hover:bg-amber-600 dark:bg-amber-600 dark:hover:bg-amber-500 text-white font-bold rounded-xl shadow-md dark:shadow-lg dark:shadow-amber-900/20 transition flex items-center justify-center gap-2 group flex-1 xl:flex-none">
                                    <BanknotesIcon class="w-5 h-5 text-amber-100 dark:text-amber-200"/> Cobrar Interés
                                </button>
                                <button @click="abrirModalPago('Capital')" 
                                    class="px-6 py-3 bg-emerald-500 hover:bg-emerald-600 dark:bg-emerald-600 dark:hover:bg-emerald-500 text-white font-bold rounded-xl shadow-md dark:shadow-lg dark:shadow-emerald-900/20 transition flex items-center justify-center gap-2 group flex-1 xl:flex-none">
                                    <CurrencyDollarIcon class="w-5 h-5 text-emerald-100 dark:text-emerald-200"/> Amortizar
                                </button>
                            </template>
                        </div>
                    </div>
                    ...
                    <div class="grid grid-cols-1 xl:grid-cols-3 gap-8">
                        ...
                        <div class="xl:col-span-1 space-y-6">
                            ...
                            <div class="bg-white dark:bg-[#1a1a1a] rounded-[24px] p-6 border border-gray-200 dark:border-gray-800 shadow-sm dark:shadow-xl">
                                ...
                                <div class="space-y-4">
                                    <div class="flex justify-between items-end">
                                        ...
                                        <span class="text-2xl font-black text-gray-900 dark:text-white">{{ dinero(selectedLoan.monto) }}</span>
                                    </div>
                            ...
                            <div class="bg-white dark:bg-[#1a1a1a] rounded-[24px] p-6 border border-gray-200 dark:border-gray-800 shadow-sm dark:shadow-xl">
                                ...
                                <div class="space-y-3">
                                    <div v-for="art in selectedLoan.articulos" :key="art.id" 
                                        class="flex gap-4 items-center p-3 rounded-2xl bg-gray-50 dark:bg-[#141414] hover:bg-gray-100 dark:hover:bg-[#181818] transition border border-gray-200 dark:border-gray-800">
                                        <div class="w-14 h-14 rounded-xl bg-white dark:bg-black border border-gray-200 dark:border-gray-800 flex items-center justify-center overflow-hidden shrink-0">
                                            <img v-if="art.foto_url" :src="`/storage/${art.foto_url}`" class="w-full h-full object-cover" />
                                            <PhotoIcon v-else class="w-6 h-6 text-gray-400 dark:text-gray-700" />
                                        </div>
                                        <div>
                                            <p class="font-bold text-gray-900 dark:text-gray-200 text-sm leading-tight">{{ art.nombre_articulo }}</p>
                        ...
                        <div class="xl:col-span-2">
                            <div class="bg-white dark:bg-[#1a1a1a] rounded-[24px] border border-gray-200 dark:border-gray-800 overflow-hidden shadow-sm dark:shadow-xl min-h-[400px]">
                                <div class="p-6 border-b border-gray-200 dark:border-gray-800 flex justify-between items-center bg-gray-50/50 dark:bg-[#1a1a1a]">
        ...
        <Modal :show="showPagoModal" @close="showPagoModal = false">
            <div class="bg-white dark:bg-[#1a1a1a] p-8 rounded-[24px] border border-gray-200 dark:border-gray-800 text-gray-700 dark:text-gray-200 relative overflow-hidden">
        ...
        <Modal :show="showEstadoModal" @close="showEstadoModal = false">
             <div class="bg-white dark:bg-[#1a1a1a] p-8 rounded-[24px] border border-gray-200 dark:border-gray-800 text-gray-700 dark:text-gray-200">
                ...
                <p class="text-xs font-medium text-amber-700 dark:text-yellow-500 mb-6 bg-amber-50 dark:bg-yellow-900/10 p-3 rounded-xl border border-amber-200 dark:border-yellow-900/20 flex gap-2 shadow-sm">
                    <span class="text-amber-500">⚠️</span> Cambiar el estado manualmente puede afectar la consistencia de los reportes automáticos.
                </p>
        ...
        <Modal :show="showEditarPagoModal" @close="showEditarPagoModal = false">
            <div class="bg-white dark:bg-[#1a1a1a] p-8 rounded-[24px] border border-gray-200 dark:border-gray-800 text-gray-700 dark:text-gray-200">
                ...
                <div class="space-y-4 bg-gray-50 dark:bg-black/50 p-6 rounded-2xl border border-gray-200 dark:border-gray-800 shadow-inner">
                     <div>
