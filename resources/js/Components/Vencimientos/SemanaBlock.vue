    ...
    <div class="bg-white dark:bg-[#1a1a1a] rounded-2xl border border-gray-200 dark:border-gray-800 shadow-sm dark:shadow-md overflow-hidden">
        ...
        <div class="px-5 py-3 bg-gradient-to-r from-gray-800 to-gray-900 dark:from-gray-900 dark:to-black text-white flex items-center justify-between">
            <div class="flex items-center gap-3">
                <span class="text-sm font-black uppercase tracking-widest text-amber-400">
                    Semana {{ bloque.semana }}
                </span>
                <span class="text-xs font-medium text-gray-300 border-l border-gray-600 pl-3">{{ bloque.rango }}</span>
            </div>
            <div class="flex items-center gap-3">
                <span class="text-xs font-bold text-emerald-400">{{ formatCurrency(bloque.resumen.monto_total) }}</span>
                <span class="bg-white/10 text-white text-xs font-black px-3 py-1 rounded-full border border-white/20">
                    {{ bloque.resumen.total }} cobros
                </span>
            </div>
        </div>
        ...
        <div class="grid grid-cols-7 divide-x divide-gray-100 dark:divide-gray-800">
            ...
            >
                ...
                <div :class="[
                    ...
                    <p :class="['text-xs font-extrabold uppercase tracking-wide', dia.es_hoy ? 'text-indigo-700 dark:text-indigo-400' : 'text-gray-500 dark:text-gray-400']">
                        {{ dia.nombre_dia }}
                    </p>
                    <p :class="['text-base font-black leading-tight mt-0.5', dia.es_hoy ? 'text-indigo-800 dark:text-indigo-300' : 'text-gray-800 dark:text-gray-200']">
                        {{ dia.numero_dia }}
                    </p>
                </div>
